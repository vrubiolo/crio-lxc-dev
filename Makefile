...
PKG_CONFIG := $(shell PKG_CONFIG_PATH=/usr/local/lib/pkgconfig pkg-config --libs --cflags lxc)
...
crio-lxc-init: $(GO_SRC) Makefile go.mod
	CGO_ENABLED=0 go build -tags allthreadssyscall -ldflags '$(INIT_LDFLAGS)' -o $@ ./cmd/init
